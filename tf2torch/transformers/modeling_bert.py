...
#         token_type_ids = token_type_ids.view(-1,token_type_ids.size(-1))
        ...
        #***LSTM计算***
        #pooled_output:[bath_size*split_num,config.hidden_size]即[4*4,768],
        #变换形状后变为[4,4,768]，符合LSTM的输入格式，此时将其认为是由四个字组成的一句话
        output = pooled_output.view(-1,4,pooled_output.size(-1))
        output,(hidden,_)=self.lstm(output)
